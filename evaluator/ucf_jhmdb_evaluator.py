...
class UCF_JHMDB_Evaluator(object):
    def __init__(self,
        ...
        # self.gt_file = os.path.join(data_root, 'JHMDB-GT.pkl')
        # self.testlist = os.path.join(data_root, 'testlist.txt')
        # dataset
        if metric == 'fmap':
    ...
    def evaluate_frame_map(self, model, epoch=1, show_pr_curve=False):
            ...
            with torch.no_grad():
                ...
                for bi in range(len(batch_scores)):
                    ...
                    if self.dataset == 'ucf24':
                        detection_path = os.path.join('results', 'ucf_detections', self.model_name,
                                                      'detections_' + str(epoch), frame_id)
                        current_dir = os.path.join('results', 'ucf_detections', self.model_name,
                        ...
                        detection_path = os.path.join('results', 'jhmdb_detections', self.model_name,
                                                      'detections_' + str(epoch), frame_id)
                        current_dir = os.path.join('results', 'jhmdb_detections', self.model_name,
                                                   'detections_' + str(epoch))
        ...
        metric_list, mAP= evaluate_frameAP(self.gt_folder, current_dir, self.iou_thresh,
