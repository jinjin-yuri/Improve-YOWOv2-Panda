...
class FrequencyHeadMixer(nn.Module):
    def __init__(self, num_heads, freq_bins, head_dim, expansion=2):
        super().__init__()
...
class MSFFT(nn.Module):
    def __init__(self, embed_dim, seq_len, num_heads=4, dropout=0.2):
        """
        ...
        # 新增模块
        self.head_mixer = FrequencyHeadMixer(num_heads, self.freq_bins, self.head_dim)
        print('MMMMMSSSTTT')
...
class EnhancedSpectralSE(nn.Module):
    def __init__(self, dim, seq_len=None):
        super().__init__()
        self.dim = dim
        self.seq_len = seq_len
        print('SSSSSSSSSEEEEEEEEEEE')

        # 频谱处理（含内置ModReLU）
        self.low_freq_fft = MSFFT(dim, 49, num_heads=4, dropout=0.3)
        # # ECA注意力
...
if __name__ == '__main__':
    # 初始化
    attn = EnhancedSpectralSE(dim=976)
