...
from .encoder import build_channel_encoder, build_orign_encoder
...
class YOWO(nn.Module):
    def __init__(self, 
        ...
        self.cls_channel_encoders = nn.ModuleList(
            [build_orign_encoder(cfg, bk_dim_2d[i] + bk_dim_3d, cfg['head_dim'])
                for i in range(len(cfg['stride']))])

        # [build_orign_encoder(cfg, bk_dim_2d[i] + bk_dim_3d, cfg['head_dim'])
        #
        #
        # [build_channel_encoder(cfg, bk_dim_2d[i]+bk_dim_3d, cfg['head_dim'], bk_dim_2d[i], bk_dim_3d)
