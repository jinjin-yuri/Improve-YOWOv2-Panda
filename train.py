...
def parse_args():
    ...
    parser.add_argument('--eval', action='store_true', default=True,
                        help='do evaluation during training.')  # 在训练期间进行评估。
    ...
    parser.add_argument('--max_epoch', default=30, type=int,
                        help='max epoch.')
    parser.add_argument('--lr_epoch', nargs='+', default=[3, 4, 5, 6], type=int,
    ...
    parser.add_argument('-d', '--dataset', default='ava_v2.2',
                        help='ucf24, ava_v2.2')
    parser.add_argument('--root', default='C:/jiang/YOWOv2/dataset',
                        help='data root')
...
def train():
    ...
    # writer = SummaryWriter(log_dir=r'C:\jiang\YOWOv2\PD2\test\my')  # liu 设置log的文件夹路径及名称
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for iter_i, (frame_ids, video_clips, targets) in enumerate(dataloader):
            ...
            writer.add_scalar('loss', losses.item(), ni)  # liu
            writer.add_scalar('loss_conf', loss_dict['loss_conf'], ni)  # liu
            writer.add_scalar('loss_cls', loss_dict['loss_cls'], ni)  # liu
            writer.add_scalar('loss_box', loss_dict['loss_box'], ni)  # liu
        ...
        if epoch % args.eval_epoch == 0 or (epoch + 1) == max_epoch:
            eval_one_epoch(args, model_without_ddp, evaluator, epoch, path_to_save, optimizer,writer) #liu


def eval_one_epoch(args, model_eval, evaluator, epoch, path_to_save, optimizer,writer):
    # check evaluator
    if distributed_utils.is_main_process():
        if evaluator is None:
            ...
            # evaluate
            e = evaluator.evaluate_frame_map(model_eval, epoch + 1)

            # set train mode.
            model_eval.trainable = True
            model_eval.train()
            # writer = SummaryWriter(log_dir=r'C:\jiang\YOWOv2\PD2\tiny')
            writer.add_scalar('map', e, epoch + 1)
            # tensorboard --logdir=C:\jiang\YOWOv2\PD2\test\CA

        # save model
        print('Saving state, epoch:', epoch + 1)
        weight_name = '{}_epoch_{}.pth'.format(args.version, epoch + 1)
        # path_to_save = r"C:\jiang\YOWOv2\weights\jhmdb21\yowo_v2_t_o"
